#!/usr/bin/env python3
"""
Launch file for MTC pick-and-place to trash bin.
...
Then run this:
  ros2 launch xarm_scripts pickplace_mtc_gazebo.launch.py pick_x:=-0.3 pick_y:=-0.4
"""
...
def launch_setup(context, *args, **kwargs):
    ...
    # ── MTC execution node (with full MoveIt parameters + pick coords) ──
    mtc_node = Node(
        package='xarm_scripts',
        executable='pickplace_mtc_gazebo',
        output='screen',
        parameters=[
            moveit_config_dict,
            {
                'use_sim_time': True,
                'pick_x': float(pick_x.perform(context)),
                'pick_y': float(pick_y.perform(context)),
            },
        ],
    )
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument('pick_x', default_value='-0.3',
                              description='X position of the cube to pick'),
        DeclareLaunchArgument('pick_y', default_value='-0.4',
                              description='Y position of the cube to pick'),
